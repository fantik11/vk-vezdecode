...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useState(localStorage.getItem('faster') == null ? true : localStorage.getItem('faster'));
  const [ time, setTime ] = useState(localStorage.getItem('time') || "");
  const [ selfService, setSelfService ] = useState(localStorage.getItem('selfService') || false);
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          ...
          <Checkbox 
            ...
            onToggle={() => {
              if (faster) {
                setFaster(false);
                localStorage.setItem("faster", "");
              } else {
                setTime('');
                setFaster(true);
                localStorage.setItem("faster", "1");
        ...
        <div className="Place__choice-item">
          ...
          <Checkbox checked={selfService} onToggle={() => {
            ...
            localStorage.setItem("selfService", "1");
          }} />
        </div>
        <div className="Place__choice-item">
          <h3>На месте</h3>
          <Checkbox checked={!selfService} onToggle={() => {
            ...
            localStorage.setItem("selfService", "");
          }} />
        </div>
      </div>
